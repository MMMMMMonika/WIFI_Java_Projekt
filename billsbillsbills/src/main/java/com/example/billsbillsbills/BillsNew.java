...
public class BillsNew {
    ...
    private void onAddButtonClick() throws SQLException {
        if (enterQuantity.getText().isEmpty()) {
            ...
            DBProductSelectionViewService<Product> selectedProductDBReaderService =
                    new DBProductSelectionViewService<>(selectedProductResultToList);
    ...
    private void onCreateButtonClick() throws IOException, SQLException, InterruptedException {
        ...
        InvoiceDateReaderService<Bill> invoiceDateReaderService = new InvoiceDateReaderService<>(billResultToList);
        int amountOfInvoicesWithTodaysDate = invoiceDateReaderService.readAmountOfInvoiceDatesFromDB(dbConnection) + 1;
